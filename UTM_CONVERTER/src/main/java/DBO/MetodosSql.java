...
public class MetodosSql extends Conexion {
	...
	public void insertarOmodifClientes(ArrayList<Cliente> clientes,String queryActualizadora) {
		
		String queryUpdate=queryActualizadora;
        Conexion con = new Conexion();
        ...
        try {
            ...
            for(int i=0;i<clientes.size();i++) {
            	...
            	queryUpdate=queryUpdate.replace("{site_no}", String.valueOf(clientes.get(i).getSite_no()));
            	queryUpdate=queryUpdate.replace("{latitude}", "'"+String.valueOf(clientes.get(i).getLatitud_utm())+"'");
            	queryUpdate=queryUpdate.replace("{longitude} ", "'"+String.valueOf(clientes.get(i).getLongitud_utm())+"'");
            	
            	
            	
            	 logger.info("Ejecutando :"+queryUpdate);
            	  con.statemente.executeUpdate(queryUpdate);
            	  queryUpdate=queryActualizadora;//Reset values--
